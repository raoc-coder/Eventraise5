  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await stripe.confirmCardPayment(data.client_secret!, {
        payment_method: {
          card: { token: 'tok_visa' } as any, // placeholder for minimal demo; replace with Elements for production
      ...
      <Card className="w-full max-w-md card-soft">
        ...
        <CardContent>
          <form onSubmit={submit} className="space-y-4">
            ...
            <Button type="submit" className="w-full btn-primary" disabled={loading}>{loading ? 'Processing…' : 'Donate'}</Button>
          </form>
        </CardContent>
      </Card>
    </div>
